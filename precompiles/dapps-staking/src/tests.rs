	...
	ExternalityBuilder::default().build().execute_with(|| {
		...
		precompiles()
			.prepare_test(
				TestAccount::Alex,
				precompile_address(),
				EvmDataWriter::new_with_selector(Action::ReadEraStaked).write(zero_era).build(),
			)
			.expect_cost(READ_WEIGHT)
			.expect_no_logs()
...
fn register_and_verify(developer: TestAccount, contract: H160) {
	...
	DappsStaking::register(RuntimeOrigin::root(), developer.clone().into(), smart_contract)
		.unwrap();
	...
	precompiles()
		.prepare_test(
			...
			EvmDataWriter::new_with_selector(Action::ReadStakedAmount)
				.write(Bytes(Into::<H160>::into(staker.clone()).to_fixed_bytes().to_vec()))
				.build(),
		)
		.expect_no_logs()
...
fn nomination_transfer_verify(
	...
	// Verify final state
	let will_be_unstaked = init_origin_staker_info.latest_staked_value().saturating_sub(amount) <
		MINIMUM_STAKING_AMOUNT;
	let transfer_amount =
		if will_be_unstaked { init_origin_staker_info.latest_staked_value() } else { amount };
