	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			precompiles()
				...
				.expect_cost(0) // TODO: Test db read/write costs
				.expect_no_logs()
				.execute_returns(EvmDataWriter::new().write(vec![Address(Alice.into())]).build());
		});
}
...
fn execute_leave_delegators_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let expected: crate::mock::RuntimeEvent =
				StakingEvent::DelegatorLeft { delegator: Bob.into(), unstaked_amount: 500 }.into();
			// Assert that the events vector contains the one expected
			assert!(events().contains(&expected));
		});
}
...
fn cancel_leave_delegators_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let expected: crate::mock::RuntimeEvent =
				StakingEvent::DelegatorExitCancelled { delegator: Bob.into() }.into();
			// Assert that the events vector contains the one expected
			assert!(events().contains(&expected));
		});
}
