...
# parachain
nohup ./diora -d ./data/diora --alice --force-authoring --collator --discover-local --rpc-cors=all --ws-port 9977 --rpc-port 9933 --port 40041 --chain ./config/diora-dev.json -llog=info -lruntime=debug,evm=trace --  --chain ./config/rococo-local-raw.json --discover-local --port 40042 > data/log.2022 2>&1 &
